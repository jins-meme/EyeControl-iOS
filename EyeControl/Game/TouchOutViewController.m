...
@interface TouchOutViewController () <MEMEManagerRealTimeModeDataDelegate,UIScrollViewDelegate,EyeMoveManagerDelegate,NeckManagerDelegate> {
...
@implementation TouchOutViewController
...
- (void)viewDidLoad {
    ...
    for (int i = 0; i < 2; i++) {
        ...
        if (i == 0) {
            ...
            [button setImage:[UIImage imageNamed:@"icon_start.png"] forState:UIControlStateNormal];
...
- (void)gameStart {
    DLog(@"gameStart")
    TouchOutGameViewController *touchOutGameViewController = [[TouchOutGameViewController alloc] init];
    UINavigationController *navigationController = [[UINavigationController alloc] initWithRootViewController:touchOutGameViewController];
